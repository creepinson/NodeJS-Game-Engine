...
class Window extends EventEmitter {
    ...
    constructor(id:number) {
        ...
        this.browser.on("ready-to-show",()=>{this.browser.show()});
        let bounds=this.browser.getBounds();
     ...
     */
    async update() {
        this.instructions+=`${Object.entries(this.dom).map(([id,element])=>element.instructions)}`
        await this.browser.webContents.executeJavaScript(this.instructions);
        for(let id in this.dom)this.dom[id].instructions="";
        this.instructions="";
    }
    /**
    ...
    addElement(element:Element) {
        ...
        element.id=id;
        this.dom[id]=element;
        let options={id,tag:element.tag,style:element.style,width:(<any>element).width,height:(<any>element).height};
