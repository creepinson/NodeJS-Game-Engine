import { Body, CircleBody, PolygonBody, RectBody } from './bodies'
...
export class World {
	...
	addBody(body: Body, group: string) {
		body.uid = this.uid
		body.group = group
	...
	update(options: { timeSteps?: number; timeStepLength?: number } = {}) {
		...
		for (let group in this.groups) this.groups[group].forBody((body: Body) => body.frame(this.Graphics, this))
	}
...
export class BodyGroup {
	...
	forBody(callback: (body: Body, uid: number) => void) {
...
export { Body, CircleBody, PolygonBody, RectBody }
