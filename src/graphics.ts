...
export default class Graphics {
    ...
    rect(x: number, y: number, w: number, h: number = w) {
        ...
        switch (this.RECTMODE) {
            case "CORNER":
                ...
                if (this.doStroke)
                    this.instruct(`strokeRect(${x},${y},${w},${h});`);
                break;
            case "CENTER":
                if (this.doFill)
                    this.instruct(
    ...
    ellipticalArc(
        ...
        if (!this.doFill && !this.doStroke) return;
        this.instruct(
            `ellipticalArc(${x},${y},${rx},${ry},${rotation},${startAngle},${endAngle},${antiClockwise});`
        );
    }
    /**
    ...
    vectorTransform(
        v: Vector2d,
        options: { translate?: boolean; rotate?: boolean } = {}
    ) {
