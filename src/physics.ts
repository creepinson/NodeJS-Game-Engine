...
export class World {
    ...
    removeBody(id: number) {
        if (!this.bodies.hasOwnProperty(id))
            throw new Error(`No body with a uid of ${id} exists`);
        const group = this.bodies[id].group;
    ...
    createGroup(name: string): BodyGroup {
        if (this.groups.hasOwnProperty(name))
            throw new Error(`Body group ${name} already exists`);
    ...
    update(options: { timeSteps?: number; timeStepLength?: number } = {}) {
        for (let i = 0; i < (options.timeSteps || 1); i++) {
            ...
            for (let i = 0; i < bodies.length; i++) {
                this.bodies[+bodies[i]].update(
                    this,
                    options.timeStepLength || 1
                );
                for (let j = i + 1; j < bodies.length; j++) {
                    const body1 = this.bodies[+bodies[i]],
                        body2 = this.bodies[+bodies[j]];
                    if (
                        body1.isColliding(body2) &&
                        +body1.collision(body2, this) *
                            +body2.collision(body1, this)
                    )
                        body1.repulse(body2);
                }
                if (this.bodies[+bodies[i]].removed)
                    this.removeBody(+bodies[i]);
            }
        }
        for (const id in this.bodies)
            this.bodies[id].frame(this.renderer, this);
    }

    getGroup(group: string): BodyGroup {
        if (!this.groups.hasOwnProperty(group))
            throw new Error(`Body group ${group} doesn't exist`);
...
export class BodyGroup {
    ...
    forBody(callback: (body: Body2d, id: number) => void) {
        for (const id in this.bodies) callback(this.bodies[id], +id);
    }
    count() {
...
export { Body2d, CircleBody, PolygonBody, RectBody };
